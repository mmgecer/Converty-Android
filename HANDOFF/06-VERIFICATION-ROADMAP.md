...
- F-Droid FAIL nedenleri: `com.converty.app` başka Google Play uygulamasıyla çakışıyor; public Git remote ve `main` kaynak artık vardır fakat release tag henüz yoktur. Ayrıca fastlane/fdroiddata metadata, HEIC/AVIF test fixture provenance veya `scandelete` ve temiz Linux/F-Droid scanner/lint/build henüz yoktur. Paket kimliği kullanıcıya ait namespace belirlenmeden otomatik değiştirilmemiştir.
