...
2. Yerel `main` geçmişi ve ilk commit artık vardır. Amaçlanan GitHub remote'u yapılandırılmalı, `main` push edilmeli ve gönderilen kaynakla birebir eşleşen `v0.1.0` tag'i oluşturulup push edilmelidir.
...
Apache-2.0 lisansı ve yerel production imzalama gereksinimleri çözülmüştür. Application ID çakışması, GitHub remote ve push işlemleri ile `v0.1.0` tag'i, F-Droid metadatası ve temiz Linux/F-Droid derleme ve scanner doğrulaması engel olmaya devam etmektedir.
